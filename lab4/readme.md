...
6. type in terminal `npm i nodemon -D` to install nodemon ,which restarts server while file change. -D flag indicate install in dev dependency.
7. it create node-modules folder and package-lock.json
...
```
 "scripts": {
  ...
  ```
  10. now you can start the server by typing `npm run dev` in the terrminal of the project folder